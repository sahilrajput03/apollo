...
As a reminder, mutations are operations that can modify back-end data (unlike queries). Our server's schema supports the following mutations:
...
Our `onCompleted` callback stores the user's unique ID and session token in `localStorage`, so we can load these values into the in-memory cache the next time the user visits our application. We'll add that functionality in the next section.
