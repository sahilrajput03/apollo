...
> **This chapter is currently out of date.** We will update it as soon as possible to reflect new local state APIs in Apollo Client 3.
>
> To learn more about local state, please see the "final" version of the application client in `final/client/src`, along with [Managing local state](https://www.apollographql.com/docs/react/local-state/local-state-management/).
